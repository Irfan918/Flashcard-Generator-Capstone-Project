...
import { getMyflashCards, deleteFlashCardById} from "./Localstorage";

const MyflashCards = () => {
  const navigate = useNavigate();

  const [MyFlashCard, setMyTasks] = useState([]);
  ...
  const Seeall = () => {
    setNoOfCards(noOfCards + MyFlashCard.length);
  };

  //slice method used to select and view required number of cards
  const slicedCards = MyFlashCard.slice(0, noOfCards);
                ...
                <div className="flex items-center px-3 pt-3 overflow-hidden text-ellipsis">
                  ...
                  <div className=" ml-3 truncate">
                    <h1 data-testid="cardname" className="w-14 lg:w-auto font-bold overflow-hidden text-ellipsis ">
                      {MyFlashCard[index].groupname}
                    </h1>

                    <p className="text-xs">
                      {MyFlashCard[index].flashterms.length} cards
                    </p>
                  </div>
                </div>
